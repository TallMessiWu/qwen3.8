...
**产出只写脚本执行目录的相对路径**——不只避开 `/tmp`(服务器根分区紧张),`~`、`$HOME`、
`/home/<user>/` 这类绝对家目录路径同样不许出现在交给用户跑的命令里,已被纠正两次。
用完删掉,别混进 `git status`。
...
⚠️ **脚本返回 0 不等于验证通过。** `scripts/local/run_cpu_ut.sh` 出过两个假绿:partial 模式
末尾一律 `exit 0`(路径打错、pytest 根本没跑起来也报 0);全量模式下 pytest 以退出码 4 死在
collection 时一条 FAILED 都不打印,基线比对把「零失败」读成基线里的用例都好了。两个都修了,
但教训照旧:还要读它最后几行的**结论文案**。**一个会在崩溃时报绿的验证脚本,比没有更坏**
——发现包装脚本有这种洞就当场堵上,别绕着它走。
