...
if ! grep -Fq '_uses_multimodal_rope' "$active_qwen_patch"; then
    ...
    echo "Run: python3 -m pip install --no-deps --no-build-isolation -e /home/hajimi/qwen3.8/vllm-ascend/junlin-c8-mxfp" >&2
    exit 2
fi
...
if [[ "$QUANTIZATION" == "ascend" ]]; then
    ...
    if ! grep -Fq '"qwen3_5_moe_text"' "$active_modelslim_config"; then
        ...
        echo "Pull the junlin-c8-mxfp branch (it carries the PR14238 mapping) and retry." >&2
        exit 2
    fi
