...
if ! active_vllm_ascend_root="$(python3 -c '
import importlib.util
from pathlib import Path

spec = importlib.util.find_spec("vllm_ascend")
if spec is None or spec.origin is None:
    raise SystemExit("vllm_ascend is not importable")
print(Path(spec.origin).parent)
')"; then
    echo "ERROR: cannot locate the active vllm_ascend package in this environment." >&2
    exit 2
fi
...
if ! grep -Fq '_uses_multimodal_rope' "$active_qwen_patch"; then
    ...
    echo "Run: python3 -m pip install --no-deps --no-build-isolation -e /home/hajimi/qwen3.8/vllm-ascend/junlin-qfa" >&2
    exit 2
fi
...
if [[ "$QUANTIZATION" == "ascend" ]]; then
    ...
    if ! grep -Fq '"qwen3_5_moe_text"' "$active_modelslim_config"; then
        ...
        echo "Pull the junlin-qfa branch (rebased on upstream/main with the PR14238 mapping) and retry." >&2
        exit 2
    fi
